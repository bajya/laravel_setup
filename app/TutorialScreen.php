...
class TutorialScreen extends Model
{
    use HasFactory;

     public function fetchTutorialScreens($request, $columns) {
        $query = TutorialScreen::where('status', '!=', 'delete');
    ...
    public function getImageAttribute($details)
    {
        if ($details != '') {
            //if (@getimagesize($details)) {
                return asset('uploads/tutorialscreens').'/'.$details;
            //}
        }
