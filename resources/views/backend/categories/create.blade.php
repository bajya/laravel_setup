        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{$type}} Category</li>
                </ol>
            </div>
        </div>
        <!-- Start Page Content -->
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body p-3">
                        ...
                        <h4>{{$type}} {{$categories->name ?? ''}} Category</h4>
                        <hr>
                        ...
                        <form class="form-material row form-valide" method="post" action="{{$url}}" enctype="multipart/form-data">
                            ...
                            <div class="form-group col-xxl-3 col-xl-4 col-sm-4 ">
                                <label>Select Category</label><sup class="text-reddit"> </sup>
                                <select required class="form-control form-control-line" name="parent_id" id= "parent_id">
                                <option value="0">Select Category</option>
                                @if(isset($categorieLists))
                                    @foreach($categorieLists as $row)
                                        <option value="{{$row->id}}"
                                                @if(isset($categories) && ($row->id  == $categories->parent_id))
                                                selected 
                                                @endif
                            ...
                            <div class="form-group col-xxl-3 col-xl-4 col-sm-4">
                                ...
                                <input type="text" class="form-control form-control-line" name="name" required value="{{$categories->name ?? ''}}" />
                            </div>
                            <div class="col-12">
...
<script src="{{URL::asset('/js/jquery-mask-as-number.js')}}"></script>
    <script type="text/javascript">
        ...
        //   @if($type == 'Edit')
        //         $('input[name=name]').rules('add', {remote: APP_NAME + "/admin/categories/checkCategoryName{{$categories->id}}"});
        //     @endif
        });
