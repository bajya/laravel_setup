...
@section('title', ucfirst($type).' Splash Screen')
        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ucfirst($type)}} Splash Screen</li>
                </ol>
            </div>
        </div> 
        <!-- Start Page Content -->
                        ...
                        <form class="form-material  row form-valide" method="post" action="{{$url}}" enctype="multipart/form-data">
                            ...
                            <?php /* <div class="col-md-12 p-0"> -->
                                <!-- <div class="form-group col-md-6  float-left">
                                    ...
                                    @if($type == 'add' || ($type == 'edit' && $splashscreen->image == null))
                                        <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                            ...
                                            <input type="file" required name="splashscreen_image" accept=".jpg, .jpeg, .png"> </span> <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remove</a>
                                        </div>
                                    @elseif($type == 'edit')
                                        <br>
                                        <div id="catImage">
                                            <img src="@if($splashscreen->image != null){{$splashscreen->image}}@endif" width="70" />
                                    ...
                                    <div class="form-group col-xxl-3 col-xl-4 col-sm-6">
                                        ...
                                        <div class="">
                                            <div class="input-group">
                                                ...
                                                <input type="file" id="file" required name="splashscreen_image" accept=".jpg, .jpeg, .png" class="form-control">
                                            </div>
                                        ...
                                        <div class="">
                                            <div class="input-group">
                                                <div id="image_preview">
                                                    <img height="100" width="100" id="previewing" src="@if($splashscreen->image != null){{$splashscreen->image}}@endif">
                                                </div>
                                                <input type="file" id="file" name="splashscreen_image" accept=".jpg, .jpeg, .png" class="form-control">
                                            </div>
                                            <span class="text-muted">Note: Image should be JPG, JPEG, PNG Dimension 500x500.</span>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group col-xxl-3 col-xl-4 col-sm-6 ">
                                    ...
                                    <input type="text" class="form-control form-control-line" name="splashscreen_name" value="{{old('splashscreen_name', $splashscreen->name)}}" maxlength="100"  placeholder="Please enter title">
                                </div>
                            </div>
                            <input type="hidden" name="status" value="@if(isset($splashscreen) && $splashscreen->status != null) {{$splashscreen->status}} @else active @endif">
                            <?php /*<div class="form-group bt-switch col-md-6 ">
                                <label class="col-md-4">Status</label>
                                <div class="col-md-3" style="float: right;">
                                    <input type="checkbox" @if($type == 'edit') @if(isset($splashscreen) && $splashscreen->status == 'active') checked @endif @else checked @endif data-on-color="success" data-off-color="info" data-on-text="Active" data-off-text="Inactive" data-size="mini" name="val-status" id="statusCat">
                                </div>
                            </div> */?>
                            <div class="form-group col-md-12 ">
                                <label>Description</label><sup class="text-reddit"> *</sup>
                                <textarea class="form-control form-control-line" name="description" rows="5">{{old('description', $splashscreen->description)}}</textarea>
                            </div>
                            
                            <div class="col-12 ">
                                <button type="submit" class="btn btn-success submitBtn m-r-10">Save</button>
                                <a href="{{route('splashscreens')}}" class="btn btn-inverse waves-effect waves-light">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h5 class="m-t-10 text-danger changeOffer">Are you sure you want to removed Splash Screen?.</h5>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#changeImage').click(function(){
                $('#catImage').parent().append('<div class="fileinput fileinput-new input-group" data-provides="fileinput"><div class="form-control" data-trigger="fileinput"> <i class="glyphbanner glyphbanner-file fileinput-exists"></i> <span class="fileinput-filename"><i class="fa fa-upload"></i></span></div> <span class="input-group-addon btn btn-default btn-file"> <span class="fileinput-new">Select file(Allowed Extensions -  .jpg, .jpeg, .png, .gif, .svg)</span> <span class="fileinput-exists">Change</span><input type="file" required name="splashscreen_image" accept=".jpg, .jpeg, .png"> </span> <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remove</a></div>');
            ...
            @if($type == 'edit')
             //   $('input[name=splashscreen_name]').rules('add', {remote: APP_NAME + "/admin/splashscreens/checkSplashScreen/{{$splashscreen->id}}"});
            @endif
