                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <div class="header_body">
                                <div class="delete_btn">
                                    <div class="dt-buttons">
                                        <a href="javascript:void(0)" data-href="{{route('deleteNotification')}}" class="btn btn-secondary disabled bulkAction deleteOffers">Delete</a>
                                    </div> 
                                    <!-- <div class="dt-buttons">
                                        <a href="javascript:void(0)" data-href="{{route('changeStatusOffers')}}" class="btn btn-secondary disabled bulkAction changeStatusUsersBulk">Activate/Deactivate</a>
                                    </div> --> 
                                </div>
                                <div class="dt-buttons float-right">
                            ...
                            <table id="notificationsTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        <th><div class="form-check form-check-flat selectAll"><label class="form-check-label"><input type="checkbox" class="form-check-input" name="notification_ids[]">Select</label></div></th>
                                         <th>S.No.</th>
                                        <th>Title</th>
                                        <th>Type</th>
                                        <th>Description</th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-valide" method="post" id="blockForm" action="{{route('changeStatusOffer')}}">
                        ...
                        <h5 class="m-t-10 text-danger">Are you sure you want to <span class="offerstatus"></span> Offer : <span id="statuscode"></span></h5>
    ...
    <div class="modal fade" id="confirmDeleteModal" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{-- <form class="form-valide" method="post" id="deleteForm" action="{{route('deleteNotication')}}"> --}}
                        {{csrf_field()}}
                        <input type="hidden" name="deleteid" id="deleteid">
                        <input type="hidden" name="deleteurl" id="deleteurl">
                        <h5 class="m-t-10 text-danger">Are you sure you want to delete selected Notication?</h5>
                        <button type="button" class="btn btn-secondary btn-flat cancelBtn m-b-30 m-t-30" data-dismiss="modal">Cancel</button>
...
@push('scripts')
    ...
    <script type="text/javascript">
    	$(function(){
            ...
            $(document).on('click','input[name="notification_ids[]"]',function(){
                $(document).find('input[name="notification_ids[]"]').prop('checked', $(this).prop('checked'));
                $(document).find('input[name="notification_id[]"]').prop('checked', $(this).prop('checked'));
                var length = $('input[name="notification_id[]"]:checked').length;
            ...
            $(document).on('click','input[name="notification_id[]"]',function(){
                var length = $('input[name="notification_id[]"]:checked').length;
                ...
                if(length == $('input[name="notification_id[]"]').length){
                    $(document).find('input[name="notification_ids[]"]').prop('checked', true);
                }else{
                    $(document).find('input[name="notification_ids[]"]').prop('checked', false);
                }
            ...
            $(document).on('click', '.confirmDeleteBtn', function (event, state) {
                ...
                $.ajax({
                    ...
                    error: function(data)
                    {
                        ...
                        toastr.error("Unable to delete Notication.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
                        ...
                        $.ajax({
                            ...
                            success: function(res)
                            {
                                ...
                                if(data.status == 1)
                                {
                                    ...
                                    $.each($('input[name="notification_ids[]"]:checked'), function(){
                                        $(this).prop('checked', false);
                                    });
