                        ...
                        <div class="table-responsive">
                            <div class="header_body">
                              ...
                              <!--  <div class="dt-buttons">
                                        <a href="javascript:void(0)" data-href="{{route('deleteCitiesBulk')}}" class="btn btn-secondary disabled bulkAction deleteCitiesBulk">Delete</a>
                                    </div> -->
                                    <div class="dt-buttons">
                                        <a href="javascript:void(0)" data-href="{{route('changeStatusCitiesBulk')}}" class="btn btn-secondary disabled bulkAction changeStatusCitiesBulk">Activate/Deactivate</a>
                                    </div>
                                </div>
                                <div class="dt-buttons float-right">
                                    <a href="{{route('createCities')}}" class="btn dt-button py-2">Add City</a>
                                </div>
                            </div>

                            <table id="CitiesTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                         <th><div class="form-check form-check-flat selectAll"><label class="form-check-label"><input type="checkbox" class="form-check-input" name="cities_ids[]">Select</label></div></th>
                                        <th>Country Name</th>
                                        <th>State Name</th>
                                        <th>Name</th>
                                        <th>Status</th>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(function(){
            ...
            function myDataTableFunction(){
             ...
             $('#filter_form').on('submit', function(e) {
                ...
                if ($.fn.DataTable.isDataTable("#CitiesTable")) {
                  $('#CitiesTable').DataTable().clear().destroy();
                }
                myDataTableFunction();
            });
             ...
             $(document).on('click','input[name="cities_ids[]"]',function(){
                $(document).find('input[name="cities_ids[]"]').prop('checked', $(this).prop('checked'));
                $(document).find('input[name="cities_id[]"]').prop('checked', $(this).prop('checked'));
                var length = $('input[name="cities_id[]"]:checked').length;

                if(length > 0)
                {
                    $('.deleteCitiesBulk').removeClass('disabled');
                    $('.changeStatusCitiesBulk').removeClass('disabled');
                    table.rows().select();
                }else{
                    $('.deleteCitiesBulk').addClass('disabled');
                    $('.changeStatusCitiesBulk').addClass('disabled');
                    table.rows().deselect();
                }
            });
             ...
             $('.bulkAction').click(function(){
                ...
                var ids = [];
                $.each($('input[name="cities_id[]"]:checked'), function(){
                    ids.push($(this).val());
                });
                console.log(ids);
                    ...
                    $.ajax({
                        ...
                        success: function(res)
                        {
                            ...
                            if(data.status == 1)
                            {
                                ...
                                $.each($('input[name="cities_ids[]"]:checked'), function(){
                                    $(this).prop('checked', false);
                                });
            ...
            $(document).on('switchChange.bootstrapSwitch', '.statusCities', function (event, state) {
             if (confirm('Are you sure you want to change status?')) {
                var x;
                if($(this).is(':checked'))
                    x = 'active';
                ...
                $.ajax({
                    type: "post",
                    url: "{{route('changeStatusAjaxCities')}}",
