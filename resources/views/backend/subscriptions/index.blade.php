	...
	<div class="content-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-primary">Subscriptions</h3> </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            ...
                            <div class="header_body">
                                <div class="delete_btn">
                                   <?php /* <div class="dt-buttons">
                                        <a href="javascript:void(0)" data-href="{{route('deleteSubscriptionsBulk')}}" class="btn btn-secondary disabled bulkAction deleteSubscriptionsBulk">Delete</a>
                                    </div>  */?>
                                    <div class="dt-buttons">
                                        <a href="javascript:void(0)" data-href="{{route('changeStatusSubscriptionsBulk')}}" class="btn btn-secondary disabled bulkAction changeStatusUsersBulk">Activate/Deactivate</a>
                                    </div>
                                </div>
                                <?php /*<div class="dt-buttons">
                                    <a href="{{route('createSubscriptions')}}" class="btn dt-button py-2">Add Subscription</a>
                                </div> */?>
                           ...
                           <!--  <div class="dt-buttons">
                            ...
                            <table id="subscriptionsTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        <th><div class="form-check form-check-flat selectAll"><label class="form-check-label"><input type="checkbox" class="form-check-input" name="subscription_ids[]"></label></div></th>
                                        <th>Name</th>
                                        <th>Price(INR)</th>
                                        <th>Status</th>
                                        <th>Added Date</th>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-valide" method="post" id="blockForm" action="{{route('changeStatusSubscriptionsBulk')}}">
                        {{csrf_field()}}
                        <input type="hidden" name="statusid" id="statusid">
                        <input type="hidden" name="status" id="status">
                        <h5 class="m-t-10 text-danger">Are you sure you want to <span class="usersstatus"></span> Subscription : <span id="statuscode"></span></h5>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <h5 class="m-t-10 text-danger">Are You sure you delete this subscription ?</h5>
                    <button type="button" class="btn btn-secondary btn-flat cancelBtn m-b-30 m-t-30" data-dismiss="modal">Cancel</button>
    ...
    <script type="text/javascript">
    	$(function(){
            ...
            if ($.fn.DataTable.isDataTable("#subscriptionsTable")) {
                $('#subscriptionsTable').DataTable().clear().destroy();
            }
            myDataTableFunction();
            ...
            $(document).on('click','input[name="subscription_ids[]"]',function(){
                $(document).find('input[name="subscription_ids[]"]').prop('checked', $(this).prop('checked'));
                $(document).find('input[name="subscription_id[]"]').prop('checked', $(this).prop('checked'));
                var length = $('input[name="subscription_id[]"]:checked').length;

                if(length > 0)
                {
                    $('.deleteSubscriptionsBulk').removeClass('disabled');
                    $('.changeStatusUsersBulk').removeClass('disabled');
                    table.rows().select();
                }else{
                    $('.deleteSubscriptionsBulk').addClass('disabled');
            ...
            $(document).on('click','input[name="subscription_id[]"]',function(){
                var length = $('input[name="subscription_id[]"]:checked').length;
                ...
                if(length == $('input[name="subscription_id[]"]').length){
                    $(document).find('input[name="subscription_ids[]"]').prop('checked', true);
                }else{
                    $(document).find('input[name="subscription_ids[]"]').prop('checked', false);
                }
            });
            $(document).on('switchChange.bootstrapSwitch', '.statusSubscriptions', function (event, state) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{route('changeStatusAjaxSubscriptions')}}",
                    ...
                    error: function(data)
                    {
                        ...
                        toastr.error("Unable to update subscription.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
            ...
            $(document).on('click','.deleteSubscriptions',function(){
                var id = $(this).data('id');
                $('#deleteid').val(id);
                $('#deleteurl').val("{{route('deleteSubscriptions')}}");
            ...
            $(document).on('click', '.confirmDeleteBtn', function (event, state) {
                ...
                $.ajax({
                    ...
                    error: function(data)
                    {
                        toastr.error("Unable to delete Subscriptions.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
            ...
            $('.bulkAction').click(function(){
                ...
                var ids = [];
                $.each($('input[name="subscription_id[]"]:checked'), function(){
                    ids.push($(this).val());
                });
                        ...
                        $.ajax({
                            ...
                            success: function(res)
                            {
                                ...
                                if(data.status == 1)
                                {
                                    ...
                                    $.each($('input[name="subscription_ids[]"]:checked'), function(){
                                        $(this).prop('checked', false);
                                    });
                            ...
                            error: function(data)
                            {
                                toastr.error("Unable to update Subscriptions.","Status",{
                                    timeOut: 5000,
                                    "closeButton": true,
                                    "debug": false,
                                    "newestOnTop": true,
